...
def get_args(argv=None):
    ...
    parser.add_argument('-s', type=str, required=True, help='logFile location')
...
def send_packet(sock, client_address, seq_num, ack_num, flag_seq, data_to_send):
    '''send packet'''
    flag_num = 0
...
def handle_client(server_socket,client_addr, data, web_object, conn_dict):
    ...
    if not ack_bit and syn_bit: #first handshake 
        ...
        flag_seq = '110'
        send_packet(server_socket, client_addr, seq_to_send, ack_to_send, flag_seq, data_to_send)
    else:
